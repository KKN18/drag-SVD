...
class DragSVDPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        if guidance_scale > 1:
            unconditional_embeddings = self.get_neg_embeddings(image)
            encoder_hidden_states = torch.cat([unconditional_embeddings, encoder_hidden_states])
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler")):
            ...
            latent_model_input = torch.cat([latents, vae_latent], dim=2)    
